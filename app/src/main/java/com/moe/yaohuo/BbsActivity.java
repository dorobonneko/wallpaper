...
SwipeRefreshLayout.OnRefreshListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		loadMore = (LoadMoreView) LayoutInflater.from(this).inflate(R.layout.loadmore_view,list_view,false);
		fa.addFoorView(loadMore);
		...
		progressLayout = header.findViewById(R.id.progressLayout);
...
//tv_content_summary.setText(Html.fromHtml(content.substring(index),new ImageGetter(tv_content_summary),null));
					...
					if (mode!=null)
					{
						...
						progressLayout.setVisibility(pb.VISIBLE);
...
//handler.obtainMessage(7,fi).sendToTarget();
	...
	private class Scroll extends RecyclerView.OnScrollListener
	{
		...
		public void onScrolled(RecyclerView recyclerView, int dx, int dy)
		{
			...
			if (canload&&!load&&loadMore.getState()!=LoadMoreView.State.PROGRESS&&llm.findLastVisibleItemPosition()>llm.getItemCount()-2)
				loadReply();
		}
